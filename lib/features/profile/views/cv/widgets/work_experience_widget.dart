...
class _WorkExperienceWidgetState extends ConsumerState<WorkExperienceWidget> {
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((timeStamp) {
      ref.read(workExperiencesRef.notifier).getProfileExperience();
    });
  }
      ...
      (previous, next) {
        ...
        if (next is BaseLoading) {
          ...
          if (next is BaseSuccess) {
            ref
                .read(workExperiencesRef.notifier)
                .getProfileExperience(showLoading: false);
      ...
      children: [
        SectionBarWidget(
          ...
          onTap: () async {
            final result = await context.pushNamedAsync(
              (p0) {
                return const WorkExperienceFormScreen();
              },
            );
            if (result ?? false) {
              ref.read(workExperiencesRef.notifier).getProfileExperience();
        ...
        Consumer(
          builder: (context, ref, child) {
            return ref.watch<BaseState>(workExperiencesRef).maybeWhen(
              loading: () {
                return SizedBox(
                  width: 32.r,
              ...
              success: (data, extraData) {
                final profileExperiences = data;
                return profileExperiences.isNotEmpty
                    ? ListView.separated(
