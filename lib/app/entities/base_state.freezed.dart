...
class _$BaseError<T> implements BaseError<T> {
  ...
  @override
  String toString() {
    return 'BaseState<$T>.error(data: $data)';
  }
...
abstract class BaseError<T> implements BaseState<T> {
  const factory BaseError(final T? data) = _$BaseError<T>;
...
class _$BaseSuccess<T> implements BaseSuccess<T> {
  const _$BaseSuccess({this.data, final Map<String, dynamic>? extraData})
...
abstract class BaseSuccess<T> implements BaseState<T> {
  const factory BaseSuccess(
      {final T? data,
      final Map<String, dynamic>? extraData}) = _$BaseSuccess<T>;
